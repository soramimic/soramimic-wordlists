...
- データや画像を利用するときは、上表と `image_page` に記載された出典・ライセンスに従ってください
- `image` 列にはWikimedia Commonsの写真・肖像と、本リポジトリで作成した代替画像が混在します。たとえば `scientist.csv` は取得できた人物の肖像を収録し、肖像がない人物には実在の容姿を描かない象徴カードを割り当てています
- 実在人物名を広告などの顧客誘引に使わないでください。`youtuber.csv` は活動名のみを収録し、VTuber名は非商用のファンメイド用途を想定しています
- 生成画像は実写ではなく、公式のロゴ・キャラクター画像等も使用していません。生成カードの一部は [Material Symbols](https://github.com/google/material-design-icons)(Apache License 2.0)を含みます。詳細は各リストの出典欄とADRを参照してください
- `myoji.csv` の `rank` はWikidata上の著名人数による参考値であり、名字の人口順位ではありません
