...
| description | baseball/football固有: 日本語版Wikipediaの記事冒頭から作った短い完結文。受賞・優勝・記録等の文があれば主要実績を優先し、なければ経歴・所属等を収録する。記事が無い選手は空。同じ選手IDの各表記行には同じ値を入れ、既存値を上書きせず空欄だけを `tools/enrich_player_descriptions.py` で補完する。上流の更新を明示的に反映する場合だけ `--refresh` を使う |
