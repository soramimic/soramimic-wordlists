...
| youtuber.csv | YouTuber（`scope`で日本向け/海外中心/未判定を選択可） | Wikidata、Wikipedia、公式プロフィール、[YouTube Data API v3](https://developers.google.com/youtube/v3)、Wikimedia Commons |
| vtuber.csv | VTuber（`scope`で日本向け/海外中心/未判定を選択可） | Wikidata、Wikipedia、公式プロフィール、[YouTube Data API v3](https://developers.google.com/youtube/v3)、Wikimedia Commons、[あおぎり高校](https://vhs-city.com/aogirihighschool/)、[ななしいんく](https://www.774.ai/)、[Realize Production](https://realize-pro.com/) |
| fictional_scientist.csv | AI生成した架空の科学者（経歴・業績も架空） | jiroshimaya/fictional-scientists |
...
画像URLはGitHub Actionsで毎日2,500件ずつ検査し、保存した位置から再開して約29回で
全リストを一巡します。404・410または画像でない応答を検出するとworkflowが失敗し、
検査結果は90日間Artifactに保存されます。PRでは新たに導入する画像URLだけを検査します。
