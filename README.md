...
| image, image_page | 写真のURL(Wikimedia Commons直リンクまたは本リポジトリのGitHub Releaseアセット)と、ライセンス・作者の確認先ページ(stations/baseball/football/scientist/sekitsui/fictional_scientist/fictional_anime_character)。利用時はimage_pageのクレジット条件に従うこと。**sekitsuiは実写が取れない行に限り、`class`ごとの概念イメージSVG(`class-image-v1` リリース。画像内に「イメージ」と明記)を分類単位で共有して割り当てている**(実写ではないので、実写だけが欲しい利用側は `.../releases/download/class-image-` で始まるURLを除外すること) |
...
| sekitsui.csv | 動物(脊椎動物。分類・絶滅フラグ・目/科・画像付き) | [Wikidata](https://www.wikidata.org/) (CC0) で自動更新。写真はWikimedia Commons、実写が無い行の分類イメージ画像は本リポジトリのReleaseで配布(CC0・実写ではない) |
