...
soramimic側のパーサは空行を無視するため、ファイル末尾の改行はあってもよい。
...
| description | baseball/football固有: 日本語版Wikipediaの記事冒頭から作った短い説明。受賞・優勝・記録等の文があれば主要実績を優先し、なければ経歴・所属等を収録する。カード約2行の50文字を目安とし、完結文を保つため65文字までは許容する。それを超える場合は末尾を`…`で省略する。記事が無い選手は空。同じ選手IDの各表記行には同じ値を入れ、既存値を上書きせず空欄だけを `tools/enrich_player_descriptions.py` で補完する。上流の更新を明示的に反映する場合だけ `--refresh` を使う |
