...
| description | stations固有: 日本語版Wikipediaの記事冒頭から作った短い完結文。「唯一・最端・文化財・受賞・愛称・駅名由来・作品の舞台・眺望」などの特徴文を優先し、無ければ所在地・路線の概要に開業年を添える。定型的な「駅である。」は「駅。」の体言止め。記事が無ければWikidataの日本語説明、どちらも無ければ空。通常は空欄だけを `tools/enrich_station_descriptions.py` で補完し、抽出方式を更新したときは `--refresh` で再生成する |
