...
- youtuber: Wikidataの職業(P106)がYouTuber/バーチャルYouTuberで
  ...
  **`subscribers`(メインチャンネルの登録者数)と `subscribers_as_of`(UTCの取得日)だけは
  別スクリプトで、毎回全行を上書きする**(登録者数は時変値なので、古い値を残すと
  ...
  **最大の登録者数**を採る(`channel` のメイン判定と同じ基準)。値はYouTubeが
  公開している**概数(有効数字3桁)**で、取得できない人は両列ともNA。全チャンネルの
  取得が成功してから、2列を含む一時ファイルを原子的に置換するので、途中で失敗しても
  書きかけや登録者数と取得日の不一致は残らない。
