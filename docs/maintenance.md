...
Chrome上のGeminiを候補探索に使う場合も、Geminiの回答自体は根拠にしない。本人公式
ページ、本人QIDの記事、YouTube概要欄などが同じチャンネルへ直接結び付ける場合だけ
`web_search_primary_link`として採用する。グループ共有チャンネルも公式ページが本人を
構成員として直接対応付ける場合に限り採用し、関係を根拠要約へ残す。根拠URL不足は候補
レポートへ残す。
...
追加行はJODCの和名・学名を候補にし、WoRMSの有効AphiaID側で原則 `rank=Species` と
...
- youtuber: Wikidataの職業(P106)がYouTuber/バーチャルYouTuberで
  ...
  **`channel`(メインチャンネル名)、`subscribers`(登録者数)、
  `subscribers_as_of`(UTCの取得日)は別スクリプトで、毎回全行を上書きする**
  (チャンネル名と登録者数を同じYouTubeチャンネルIDに揃え、改名も反映する。
  登録者数は時変値なので、古い値を残すと
  比較時点が分からなくなる。空欄補完のみという方針の明示的な例外。詳細は ADR 00030):
  ```sh
  ...
  **最大の登録者数**を採る(`channel` のメイン判定と同じ基準)。値はYouTubeが
  公開している**概数(有効数字3桁)**で、取得できない人は3列ともNA。全チャンネルの
  取得が成功してから、3列を含む一時ファイルを原子的に置換するので、途中で失敗しても
  書きかけやチャンネル名・登録者数・取得日の不一致は残らない。
