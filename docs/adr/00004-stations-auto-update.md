...
- `description` は日本語版 Wikipedia の記事冒頭から、先頭の駅名・読みを落とした短い完結文を作る。記事が無ければ Wikidata の日本語 description にフォールバックし、どちらも無ければ空にする。`tools/enrich_station_descriptions.py` は空欄だけを補完し、既存の説明は上書きしない
