...
- `description` は日本語版 Wikipedia の記事冒頭から、「唯一・最端・文化財・受賞・愛称・駅名由来・作品の舞台・眺望」などの特徴を含む完結文を優先して作る。特徴文が無ければ所在地・路線の概要に `opened_year` の「○年開業。」を加える。定型的な「駅である。」は「駅。」の体言止めにする。記事が無ければ Wikidata の日本語 description にフォールバックし、どちらも無ければ空にする。`tools/enrich_station_descriptions.py` は通常は空欄だけを補完し、抽出方式を更新したときは `--refresh` で再生成する
