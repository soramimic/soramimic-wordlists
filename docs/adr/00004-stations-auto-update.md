...
- `description` はカード約3行の短文を基本とする。日本語版 Wikipedia の記事冒頭から、「唯一・最端・文化財・受賞・愛称・駅名由来・作品の舞台・眺望」などの特徴を含む文を最優先し、定型句や元号併記を圧縮する。特徴文は48字を機械的に超えたという理由で捨てず、表示側の自動フィットに任せる。特徴が無い駅はカード上部の都道府県・路線表示との重複を避け、`opened_year` の「○年開業。」だけにする。定型的な「駅である。」は「駅。」の体言止めにする。Wikipedia の特徴文が無い場合は Wikidata の日本語 description に特徴があれば採用し、どちらにも特徴が無く開業年も無ければ空にする。`tools/enrich_station_descriptions.py` は通常は空欄と旧方式の省略文だけを補完し、抽出方式を更新したときは `--refresh` で再生成する
