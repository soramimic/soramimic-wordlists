...
- Date: 2026-07-28
- Supersedes: none
- Superseded by: [00019](00019-youtuber-derived-colors.md) (Decision 5 のうち「キャラクターイラスト/アバターからのパレット抽出はしない」という判断のみが置き換え。他は有効)
...
- ~~**キャラクターイラスト/アバターからのパレット抽出はしない**。イラストは創作的表現であり、
  ...
  (この判断は [ADR 00019](00019-youtuber-derived-colors.md) で撤回した。著作権法30条の4に
  照らすと、代表色を求める処理は情報解析であり、享受を目的としない限り認められる)
...
ホロライブは公式サイトのHTML/CSS/JSONにタレント色のフックが無く、色を出しているのは大型ライブの
公式ペンライトカラー一覧画像だけなので、そこから読む。7th fes.(2026)を主に使い、そこに出ていない
人は 6th fes.(2025)で埋める。ペンライトカラーは公演ごとの割り当てなので稀に変わる
(IRyS は 6th=VIOLET, 7th=PINK)。**新しい方を優先**し、先に入った色を残す。
12色の離散パレット(RED/BLUE/WHITE/…)なので同じ色になる人が出るが、事務所ハッシュで全員同系色に
なる従来よりは確実に情報量が増える。

取得できないことを確認したもの。推測で埋めるくらいならフォールバックの色を使う:

- **にじさんじ**: 公式ペンライトカラーは**ライブの日程ごとの公式X投稿の本文でしか公表されておらず**、
  にじフェスの特設サイト(`fes.nijisanji.jp`)にはペンライトの記載自体が無い。しかも公表されるのは
  「HOT PINK」のような色名だけで、色名と実際の色を対応づける色見本も公式には出ていないため、
  hexにできない
...
- イメージカラーが付くのはカード対象712人中68人(ホロライブ52 / あおぎり高校12 /
  ぶいすぽっ！2 / Neo-Porte 2)。カード対象のホロライブ勢はほぼ全員に色が付く
