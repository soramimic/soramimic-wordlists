...
海水域を生活場所に含む、一般に知られた現生の海洋動物を収録する。分類群を
網羅するリストではなく、動画で扱いやすい語をレビューしたテーマ別リストである。
...
| order, family | 目と科。台帳で確認できた日本語分類名 |
| description | 各生物に固有の生態または形態を表す20〜60字の完結した短文 |
| image, image_page | 分類別の概念画像と本リポジトリ上の確認先 |
| wikidata | 対応を確認できた分類群のQID。未確認の場合は空欄 |

画像は個々の生物の実写ではなく、4分類を表す共通の図である。画像内に
「イメージ」と表示し、特定の種の姿と誤認しないようにしている。
...
| 列 | 意味 |
|---|---|
| verified | 実在人名リストで同じ表記と読みを確認できたか(`yes`/`no`) |
