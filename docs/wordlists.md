...
海水域を生活場所に含む現生の海洋動物1000件を収録する。分類群を網羅するリストではなく、
初版179件と、JODCの和名・学名をWoRMSの有効AphiaIDで海洋性確認した821件からなる。
...
528件はCommonsの自由ライセンス実写（幅960px以下の配信用サムネイル）、残る472件は分類別の自作概念SVGである。
