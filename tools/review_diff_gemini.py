...
- 人名リスト(baseball.csv / football.csv / scientist.csv):
  ...
  明らかなゴミ文字列が混入していないか
- その他のリスト(stations/pokemon/nations/sekitsui/plant/youtuber):
  追加行を確認し、全体として異常(大量のゴミ行、日本語名でない行、
  カテゴリ違いの混入)がないか
- 既存行の変更(-行と+行の対): status更新や画像列(image/image_page)の付与など
  想定内の変更だけか。original/surface/読みの書き換えや行の削除があればNG
