...
- 既存行の変更(-行と+行の対): status更新・画像列(image/image_page)の付与・
  subscribers列の数値更新など想定内の変更だけか。original/surface/読みの
  書き換えや行の削除があればNG
...
- youtuber.csvのsubscribers列(チャンネル登録者数)は時変値で、毎月の実行で
  ほぼ全行(700行規模)の数値が一斉に更新される(ADR 00030の仕様)。変更行数の
  多さや変動幅の大きさはNGの理由にしない。subscribers以外の列が同じ行で
  一緒に書き換わっていないかだけを確認する

判定基準: 上記仕様に該当するものはOK扱い。それ以外で明確な誤り(姓名の逆転、
読みと名前の明白な不整合、ゴミ混入、既存行の破壊的変更)があればNG、迷ったらNG。
