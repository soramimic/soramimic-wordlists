#!/usr/bin/env python3
"""象徴カード・選手カードの地紋に敷く「職業アイコン」。

カードは名前・区分・所属の文字を描かないので、そのままだと「サッカー選手」と
「サッカー監督」と「クラブマスコット」が配色と頭文字だけになって見分けが
付かない。区分の文字の代わりに**職業が想像できるアイコン**をカードの地紋
(ウォーターマーク)として敷き、文字を増やさずに区別する。
...
見えたので、造形はプロの素材に任せることにした(ADR 00024)。
